  ...
  return (
    <div className="flex h-screen w-full flex-col overflow-hidden text-zinc-900 dark:text-zinc-100">
      ...
      <main className="mx-auto flex min-h-0 w-full max-w-7xl flex-1 flex-col gap-4 overflow-hidden px-2 pb-0 md:px-4 md:flex-row md:gap-8 md:pb-8 2xl:max-w-[1600px]">
        <section className="scrollbar-thin min-h-0 w-full overflow-y-auto py-2 px-1 pb-40 md:pb-2 md:min-w-0 md:w-[42%] md:flex-none md:rounded-2xl md:bg-zinc-100 md:p-5 md:shadow-sm dark:md:bg-zinc-900/95">
          <IntervalEditorList />
        </section>

        <section className="scrollbar-thin hidden min-h-0 min-w-0 flex-1 overflow-y-auto rounded-2xl bg-primary-600 p-5 text-white shadow-lg dark:bg-zinc-900 dark:text-zinc-50 md:block md:p-6">
          <PlayerPanel />
        </section>
      </main>
      <MobilePlayerBar />
...
function MobilePlayerBar() {
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 flex flex-col items-center gap-4 pb-2 pt-12 md:hidden">
      <button
        type="button"
        onClick={
          player.isRunning
        ...
        disabled={!player.isRunning && playbackIntervals.length === 0}
